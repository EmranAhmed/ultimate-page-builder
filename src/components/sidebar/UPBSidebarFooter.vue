<template>
    <div id="upb-sidebar-footer">
        ...
        <div class="previews">
            ...
            -->
            <a v-for="device in devices" :class="{ active: currentDevice(device.id) }" :title="device.title" @click.prevent="toggleResponsivePreview(device)" href="#"><i :class="device.icon"></i></a>
        </div>
    </div>
</template>
